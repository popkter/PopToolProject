...
应用会在用户主动使用 Jira 飞书推送、安装可选 PowerShell 7 插件、安装 Python
依赖或检查软件更新时访问对应服务。客制脚本是否访问网络取决于脚本本身。
...
- **记录日志与视频**：录制 Android 设备画面、系统声音和麦克风，并同步保存 logcat 日志。开始采集前会先清空设备的 logcat 缓冲区。需要 Android 11 或更高版本，设备还需支持 `VOICE_PERFORMANCE` 音源；部分应用可能禁止内部音频采集。
...
1. 进入“预设 → Jira 飞书推送”。
2. 从顶部方案列表选择已有方案，或使用“新建”“复制”创建独立方案。切换方案前，当前表单内容会保留在对应方案中。
3. 在“Jira”页填写 Jira 地址、Token/PAT、JQL 查询和最多获取数，然后点击“测试连接”验证 Jira 凭据与查询。
4. 在“飞书”页填写群机器人的 Webhook URL。机器人启用了安全关键词或签名校验时，同时填写关键词或签名 Secret。
5. 如需在互动卡片中真正 `@` 负责人，请填写邮箱域名以及飞书自建应用的 App ID、App Secret；无法解析 `open_id` 时，卡片只显示负责人姓名，不会发送无效的 `@`。
6. 点击“预览消息”检查卡片内容，确认后点击“保存配置”或“立即推送”。底部运行记录可展开查看测试、预览和推送结果。

每个方案分别保存 Jira、飞书、消息与定时设置。Token、Webhook、签名 Secret 和飞书应用凭据保存在本机用户数据目录的 `jira_feishu` 配置中，请妥善保护该目录及电脑账户。

“定时”页支持按分钟间隔或每日多个时刻推送。多个每日时刻使用逗号分隔，例如 `09:00,15:00,18:00`。调度器只在泡泡工具箱进程运行期间工作；关闭窗口隐藏到系统托盘后仍可继续，完全退出应用后停止。
...
- **主题**：跟随系统、浅色或深色；
- **主题风格**：Material 3 或 Windows XP。

修改后会立即应用到主界面、弹窗和控件状态，并保存为下次启动的默认设置。主题切换只影响界面外观，不会更改客制脚本、Jira 飞书方案或 Android 设备配置。

## 检查应用更新

发布版会在冷启动后异步检查更新，同一次成功检查后的 24 小时内不再重复请求。自动检查失败不会阻断启动，也不会影响本地工具。

进入“设置 → 应用更新”可以随时点击“检查更新”。默认只接收 GitHub 最新正式版；开启“接收测试版本”后，会读取最近发布的 5 条 Release（包括 prerelease），按语义版本号选择其中最高版本，避免发布时间顺序与版本号顺序不一致时选错版本。发现新版本后才由用户确认下载更新包。
...
工具配置、颜色处理、Android 操作和脚本内容都保存在本机。使用 Jira 飞书推送时，应用会按当前方案访问 Jira、飞书机器人及可选的飞书开放平台；安装插件、Python 依赖和检查更新时也会访问对应下载服务。客制脚本可能按其内容访问其他网络地址。
...
- **Jira 测试连接失败**：确认 Jira 地址、Token/PAT 和 JQL 可用，并检查当前网络是否能访问 Jira 服务。
- **飞书推送失败**：确认 Webhook、关键词及签名 Secret 与机器人安全设置一致；需要 `@` 负责人时，再检查 App ID、App Secret 和邮箱域名。
- **定时推送没有执行**：确认目标方案已启用定时、调度器已经启动，并保持泡泡工具箱在前台或系统托盘中运行。
- **关闭窗口后程序仍在运行**：这是系统托盘行为，请从托盘菜单完全退出。
- **同时运行的任务达到上限**：应用会提示停止最早启动的普通任务；Android 投屏保留独立运行名额。
- **检查不到测试版本**：进入“设置 → 应用更新”开启“接收测试版本”，再点击“检查更新”；关闭时只检查最新正式版。
