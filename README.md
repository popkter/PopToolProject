...
- **Android 设备**：自动扫描设备，每个脚本、预设命令、投屏和录屏分别选择并记住目标设备。
...
4. 使用 Android 功能前，在设备上开启 USB 调试并授权电脑，然后在该功能的“目标设备”下拉框中选择设备。
...
系统托盘提供预设工具和最近使用自定义脚本的快捷入口。有参数或使用 Android 设备的脚本会先显示配置窗口，其他无参数脚本直接运行。
...
需要设备的命令使用该命令的“目标设备”选择。持续运行的 Logcat 可再次点击运行按钮停止。
...
每个工具独立保存设备选择，分享脚本不会携带设备记录。首次只有一台可用设备时自动选择，多台时手动选择。选中设备断开后不会自动切换到其他设备，重连后可继续使用；也可展开下拉框刷新或重新选择。
