...
| 预设 | 使用 Android 投屏与录制、Jira 飞书推送和调色盘等内置工具 |
...
- **Jira 飞书推送**：按 JQL 拉取 Jira Issue，按负责人生成飞书互动卡片；支持多推送方案、连接测试、消息预览、立即推送及间隔/每日定点调度。Jira Token、飞书 Webhook 与应用凭据仅保存在本机用户数据目录。
...
脚本中的占位符会自动转换为运行前的输入框或选择框。控件标题可以使用中文、字母、数字和下划线。
...
使用 `${标题:选项文字=实际值|选项文字=实际值}` 创建选择框。界面显示选项文字，执行时替换为对应的实际值，第一项为默认选项：

```powershell
adb shell settings put system show_touches ${触摸点显示:开启=1|关闭=0}
```

选择“开启”时命令中的占位符会替换为 `1`；选择“关闭”时会替换为 `0`。选择框标题始终为“触摸点显示”，不会使用某个选项的名称。

普通文本输入框的内容与当前默认值不同时，输入框右侧会显示“设为默认值”。点击后会把当前内容写回客制脚本的模板定义；例如输入 `2` 后，`${请输入数值:1}` 会更新为 `${请输入数值:2}`。该功能只对可编辑的客制文本参数开放。
...
- `1786356497528` 是默认值；
- `pVal` 声明行不会交给脚本解释器执行；
- `pVal timestampMs: ${时间戳:...}` 也可用于声明变量；
- 旧版 `${变量名=默认值}` 和 `${开启=1|关闭=0}` 模板仍然兼容，但新脚本建议统一使用冒号分隔标题与定义。
...
工具执行、颜色处理、Android 操作和脚本内容都在本机完成。网络只用于经用户确认的插件安装，以及发布版的软件更新检查与下载。
