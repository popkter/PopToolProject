...
泡泡工具箱 1.1.3 是一款面向 Android 开发与测试的 Windows/macOS 桌面工具箱，用于集中管理内置能力、个人脚本、Android 设备和交互式终端。

应用数据默认保存在本机。自定义脚本、参数和执行输出不会作为工具内容上传；Jira 飞书推送、PowerShell 7 插件安装、Python 依赖安装和应用更新会在用户使用对应功能时访问相关服务。

> 本文适用于当前 1.1.3 源码版本；已安装版本的功能以对应发布版本为准。

## 主要功能

- **自定义脚本**：在双栏工作区中创建、编辑、删除、搜索、筛选和运行 PowerShell、Bash、Batch、Python 脚本。
- **运行参数**：从脚本占位符自动生成文本、选项、文件或文件夹选择控件；参数文本框支持将当前输入保存为默认值。
...
2. 首次启动后阅读应用内引导。
   引导内嵌离线 HTML 手册，支持模板编辑、参数预览和结果复制。阅读后点击右上角“关闭”，
   后续启动不再自动显示；随时可从“设置 → 关于 → 用户帮助”重新打开。
...
`Var` 声明行只用于参数元数据，不会交给脚本解释器执行。文件、文件夹参数也支持这种声明方式，例如 `Var logs = ${日志目录@dir:C:\Logs}`，脚本正文中使用 `${logs}` 引用。旧版脚本中的 `pVal` 声明仍可继续使用，但新脚本统一推荐使用 `Var`。
...
自定义脚本详情页和预设命令参数页的所有文本参数框均提供保存图标，鼠标悬停显示“设为默认值”。适用于普通文本、数字、密码、文件路径、文件夹路径和多行文本参数；下拉选项和开关不使用该按钮。
...
拖动窗口边缘或四角调整尺寸时，优先使用系统原生缩放；不支持时一次性更新窗口位置和宽高，减少对角线拖动产生的中间尺寸和重复绘制。原有界面布局、拖动区域与尺寸限制保持不变。
