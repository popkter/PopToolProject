...
- 在模型、插件、执行管理器和 QML 初始化前，通过 QLockFile 选举主实例，QLocalServer/QLocalSocket 传递版本化请求并确认接收。身份包含 SID、Windows 登录会话、实际权限和规范化数据目录，连接双方检查管道对端进程身份。
- 普通启动只激活窗口；右键请求每次创建独立的 Power'Shell 标签。最小化时恢复，前台激活失败时提醒任务栏。启动队列保持顺序，同一请求 ID 重试去重，活实例无响应不会另开主窗口；崩溃遗留锁由 QLockFile 在进程退出后恢复。
- AppDialog 和原生文件选择器统一登记模态状态，后台建标签与选中/焦点分离，全部弹窗关闭后显示最后创建的标签。缺少运行时的目录请求仅存内存，用户安装后主动打开终端才处理，退出清空。
- 管理员使用普通数据目录下的 administrator 子目录，不复制或同步脚本、设置、日志、插件或 Python 环境，窗口标题标注管理员。不会自动提升权限。
- 定向验证：5 项实例测试（并发冷启动 8 进程、请求重试、就绪前排队、活实例无响应、崩溃恢复、配置目录隔离和协议拒绝）；6 项运行时测试（弹窗草稿/焦点、多弹窗延迟、最小化恢复、运行时缺失队列、路径解析及标签关闭/可见性）。全部通过。真实 Debug 主应用隔离数据目录启动与第二进程转发均退出 0。
- 尚未实测：真实普通/管理员窗口同时运行及跨权限拒绝、系统禁止前台激活时的任务栏提醒；代码包含相应身份校验及提醒分支。协议队列上限 128，去重缓存最近 4096 个 ID，超过容量报告失败；内存请求不提供崩溃后恢复。

## PowerShell 历史预测开关（2026-09-14）

- 终端右键菜单提供“显示历史 / 关闭历史”，状态写入设置并作用于当前窗口内已有及后续创建的全部 PowerShell 会话。
- 启动脚本通过 PSReadLine 专用控制键静默设置 `History + ListView` 或 `None + InlineView`。私有 OSC 消息只传递提示符、成功和失败状态，由终端解析层消费，不进入屏幕或命令历史。
- 命令或子程序运行期间暂存切换，回到 PowerShell 提示符后应用；提示符上已有的输入和光标不受影响。PSReadLine 不支持时保留设置并显示一次错误。
- 定向验证覆盖持久化、菜单文字、分片控制消息、两窗格同步、新窗格继承、半条输入保留、长命令延迟、Ctrl+C/Ctrl+L，以及 PowerShell 补全和 Python REPL 返回。
