...
Item {
    ...
    // The menu is a full-size overlay above the editor, so primary-button
    // events must be forwarded explicitly to preserve normal focus/cursor
    // behavior for the covered TextField/TextArea.
    TapHandler {
        acceptedButtons: Qt.LeftButton
        gesturePolicy: TapHandler.WithinBounds
        onTapped: function(eventPoint) {
            root.target.forceActiveFocus()
            if (root.target.positionAt) {
